...
// charge la T3C (format: mot;hexsha) dans une hashtable
hashtable_t *load_t3c(const char *t3c_path) {
    ...
    while (getline(&line, &len, f) != -1) {
        // couper à partir de le première occurence de ';'
        char *sep = strchr(line, ';');
        if (!sep) continue;
        *sep = '\0';
        char *word = line;
        char *hash = sep + 1;
        // réduit la nouvelle ligne depuis le hash
...
// lit des hashes depuis stdin et affiche mot si trouvé
